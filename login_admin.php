...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $codigo_estudiante = $_POST['cedula'] ?? '';
    ...
    try {
        $stmt = $conn->prepare("SELECT e.id_estudiante, e.nombre, e.cedula, c.nombre_carrera AS carrera_nombre
                                FROM estudiantes e
    ...
    header('Location: login.php'); // Redirect if not a POST request
    exit();
}
?>
